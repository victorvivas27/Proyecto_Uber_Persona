...
public class ConductorController implements IConductorController {
    ...
    @Override
    @PostMapping("/crear")
    public ResponseEntity<ApiResponse<ToConductorSalida>> crearConductor(
            @Valid @RequestBody ToConductorEntrada toConductorEntrada) {

        ToConductorSalida toConductorSalida = conductorServise.crearConductor(toConductorEntrada);

        return ResponseEntity.status(HttpStatus.CREATED).body(new ApiResponse<>(
                ConstantesConductor.CONDUCTOR_CREADO,
    ...
    public ResponseEntity<ApiResponse<List<ToConductorSalida>>> listarConductor() {
        ...
        ApiResponse<List<ToConductorSalida>> response = new ApiResponse<>(
                ConstantesConductor.LISTA_CONDUCTOR,
                HttpStatus.OK.value(),
                toConductorSalidas
        );
    ...
    public ResponseEntity<ApiResponse<ToConductorSalida>> buscarConductorID(
            @PathVariable Long idConductor) throws ResourceNotFoundException {

        ToConductorSalida toConductorSalida = conductorServise.obtenerConductorPorId(idConductor);

        ApiResponse<ToConductorSalida> response = new ApiResponse<>(
                ConstantesConductor.CONDUCTOR_ENCONTRADO,
                HttpStatus.OK.value(),
                toConductorSalida
        );
    ...
    public ResponseEntity<ApiResponse<ToConductorSalida>> modificarConductor(
            @Valid @RequestBody ToConductorModificar toConductorModificar) throws ResourceNotFoundException {

        ToConductorSalida toConductorSalida = conductorServise.actualizarConductor(toConductorModificar);

        ApiResponse<ToConductorSalida> response = new ApiResponse<>(
                ConstantesConductor.CONDUCTOR_MODIFICADO,
                HttpStatus.OK.value(),
                toConductorSalida
        );
    ...
    public ResponseEntity<ApiResponse<Long>> eliminarConductor(
            @PathVariable Long idConductor) throws ResourceNotFoundException {

        conductorServise.eliminarConductor(idConductor);

        ApiResponse<Long> response = new ApiResponse<>(
                ConstantesConductor.CONDUCTOR_ELIMINADO,
                HttpStatus.OK.value(),
                idConductor
        );
